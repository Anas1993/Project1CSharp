...
{
    public partial class Admin___Student___Edit : Form
    {
        public Admin___Student___Edit()
        {
            InitializeComponent();
        }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (category.Text == "ID")
            {
                ...
                connection.Open();
                String query = String.Format("Select ID From Student Where ID = {0}", searchbox.Text);
        ...
        private void multirecord_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (category.Text == "ID")
            {
                SqlConnection connection = new SqlConnection(Program.CONNECTIONSTRING);
                connection.Open();
                String query = String.Format("Select * From Student Where ID = {0}", multirecord.Text);
                SqlCommand cmd = new SqlCommand(query, connection);
                SqlDataReader dr = cmd.ExecuteReader();
...
WHERE CustomerName='Alfreds Futterkiste';
                 */
                string queryx = string.Format("UPDATE Login SET ID={0},Password='{1}',UserType='{2}' WHERE ID={0}", multirecord.Text, password1.Text, "Student");
            ...
            // regular query..
            connection.Open();
            string query = string.Format("UPDATE Student SET ID={0},Student_FNAME='{1}',Student_LNAME='{2}',Student_Email='{3}',Student_Phone='{4}',Date_of_Birth='{5}',NIC='{6}',Gender='{7}',Blood_Group='{8}',Fathers_Name='{9}',Fathers_Phone_Number='{10}',Home_Address='{11}',Home_City='{12}',Home_County='{13}' WHERE ID={0}", multirecord.Text, firstname.Text, lastname.Text, emailaddress.Text, phonenumber.Text, dateofbirth.Value.Date, (nic1.Text + nic2.Text + nic3.Text), gender, bloodgroup.Text,fathername.Text,fatherphone.Text, address.Text, city.Text, country.Text);
